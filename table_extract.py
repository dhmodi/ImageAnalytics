...
def ocr_cell(im, cells, x, y):
    ...
    ftif = "%s.tif" % fbase
    ftxt = "%s.txt" % fbase
    cmd = "C:/Program Files (x86)/Tesseract-OCR/tesseract.exe %s %s" % (ftif, fbase)
...
def split_pdf(filename):
    ...
    prefix = filename[:-4]
    print ("Converting")
    cmd = "C:/Program Files/ImageMagick-6.9.8-Q8/convert.exe -density 600 %s working/%s-%%d.png" % (filename, prefix)
    print ("Converted")
    subprocess.call(cmd)
...
def extract_pdf(filename):
    ...
    for pngfile in pngfiles:
        ...
        os.system("del working/*.tif")
        os.system("del working/*.txt")
    # removde split pages
    os.system("del working/*")
    return data
